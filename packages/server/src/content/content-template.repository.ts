...
export class ContentTemplateRepository {
    ...
    getFormationTemplate(formationId) {
        return this.formationRegistry.getFormationTemplate(formationId);
    }
    /** 列出阵法模板。 */
    ...
    normalizeItem(item) {
        return this.itemRegistry.normalizeItem(item);
    }
    ...
    getItemSortLevel(item) {
        return this.itemRegistry.getItemSortLevel(item, (techniqueId) => this.techniqueRegistry.getTechniqueRealmLevel(techniqueId));
    }
    
    createTechniqueState(techniqueId) {
        return this.techniqueRegistry.createTechniqueState(techniqueId);
    }
    
    getTechniqueName(techniqueId) {
        return this.techniqueRegistry.getTechniqueName(techniqueId);
    }
    ...
    hydrateTechniqueState(input) {
        ...
        return techId ? this.techniqueRegistry.hydrate(techId, input) : null;
    }
    ...
    createRuntimeMonstersForMap(mapId) {
        return this.monsterTemplateRegistry.createRuntimeMonstersForMap(mapId, (targetMapId) => this.buildFallbackMonsterRuntimeStatesForMap(targetMapId));
    }
    /**
 ...
 */
    ...
    createRuntimeMonsterSpawn(monsterId, options: any = {}) {
        return this.monsterTemplateRegistry.createRuntimeMonsterSpawn(monsterId, options);
    }
    ...
    getSkillRef(skillId) {
        return this.skillRegistry.getSkillRef(skillId);
    }
    
    loadSharedTechniqueBuffs() {
        this.buffRegistry.loadSharedTechniqueBuffs();
    }
    ...
    loadAll() {
        ...
        this.monsterTemplateRegistry.monsterRealmBaselines = this.monsterRealmBaselines;
        this.loadMonsterDrops();
        this.buffRegistry.registerTemplates(Array.from(this.itemTemplates.values()).flatMap((item) => item.consumeBuffs ?? []));
