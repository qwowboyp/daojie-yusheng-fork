...
const INSTANCE_MONSTER_RUNTIME_WORKER_DOMAIN = 'monster_runtime';
const INSTANCE_MONSTER_RUNTIME_WORKER_IDLE_MS = 2_000;
const INSTANCE_MONSTER_RUNTIME_WORKER_CLAIM_LIMIT = 32;

interface InstanceMonsterRuntimeFlushRuntimePort {
...
@Injectable()
export class InstanceMonsterRuntimeFlushWorker {
  private readonly logger = new Logger(InstanceMonsterRuntimeFlushWorker.name);

  constructor(
    @Inject(WorldRuntimeService)
    private readonly worldRuntimeService: InstanceMonsterRuntimeFlushRuntimePort,
  ...
  async runOnce(workerId: string): Promise<number> {
    ...
    for (const entry of claimed) {
      ...
      try {
        const result = await this.worldRuntimeService.flushInstanceDomains?.(instanceId, [INSTANCE_MONSTER_RUNTIME_WORKER_DOMAIN]);
        ...
        this.logger.warn(
          `實例妖獸運行態 worker 刷盤失敗 instanceId=${instanceId} domain=${INSTANCE_MONSTER_RUNTIME_WORKER_DOMAIN}: ${
  ...
  async runLoop(workerId: string, idleMs = INSTANCE_MONSTER_RUNTIME_WORKER_IDLE_MS): Promise<void> {
    while (true) {
  ...
  private resolveDirtyInstances(): string[] {
    ...
    if (Array.isArray(domainEntries)) {
      return domainEntries
        .filter((entry) => Array.isArray(entry?.domains) && entry.domains.includes(INSTANCE_MONSTER_RUNTIME_WORKER_DOMAIN))
...
function resolveIdleMs(value: number): number {
  if (!Number.isFinite(value)) {
    return INSTANCE_MONSTER_RUNTIME_WORKER_IDLE_MS;
  }
