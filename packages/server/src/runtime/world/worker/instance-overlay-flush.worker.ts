...
const INSTANCE_OVERLAY_WORKER_DOMAIN = 'overlay';
const INSTANCE_OVERLAY_WORKER_IDLE_MS = 2_000;
const INSTANCE_OVERLAY_WORKER_CLAIM_LIMIT = 32;

interface InstanceOverlayFlushRuntimePort {
...
@Injectable()
export class InstanceOverlayFlushWorker {
  private readonly logger = new Logger(InstanceOverlayFlushWorker.name);

  constructor(
    @Inject(WorldRuntimeService)
    private readonly worldRuntimeService: InstanceOverlayFlushRuntimePort,
  ...
  async runOnce(workerId: string): Promise<number> {
    ...
    for (const entry of claimed) {
      ...
      try {
        ...
        this.logger.warn(
          `實例 overlay worker 刷盤失敗 instanceId=${instanceId} domain=${INSTANCE_OVERLAY_WORKER_DOMAIN}: ${
  ...
  async runLoop(workerId: string, idleMs = INSTANCE_OVERLAY_WORKER_IDLE_MS): Promise<void> {
    while (true) {
  ...
  private resolveDirtyInstances(): string[] {
    ...
    if (Array.isArray(domainEntries)) {
      return domainEntries
        .filter((entry) => Array.isArray(entry?.domains) && entry.domains.includes(INSTANCE_OVERLAY_WORKER_DOMAIN))
...
function resolveIdleMs(value: number): number {
  if (!Number.isFinite(value)) {
    return INSTANCE_OVERLAY_WORKER_IDLE_MS;
  }
