import { normalizePlantingJob } from '../craft/planting-job.helpers';
/**
  ...
  // 关键分支按状态与边界条件处理，非法路径会被提前拦截。
    ...
    createFreshPlayer(playerId, sessionId) {
        ...
        const player = {
            ...
            plantingSkill: createCraftSkillState(resolveInitialCraftSkillExpToNext(this.playerProgressionService)),
    ...
    buildLeaderboardProjectionFromSnapshot(playerId, snapshot) {
        ...
        try {
            ...
            const player = {
                ...
                plantingSkill: normalizeCraftSkillState(snapshot.progression?.plantingSkill, (level) => resolveCraftSkillExpToNextByLevel(this.playerProgressionService, level)),
 ...
 */
  ...
  // 关键分支按状态与边界条件处理，非法路径会被提前拦截。
        ...
        const player = {
            ...
            plantingJob: normalizePlantingJob(snapshot.progression?.plantingJob),
...
function cloneRuntimePlayerState(player) {
    return {
        ...
        plantingSkill: cloneCraftSkillState(player.plantingSkill),
        ...
        plantingJob: player.plantingJob ? normalizePlantingJob(player.plantingJob) : null,
