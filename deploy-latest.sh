...
# ============================================================
# 道劫余生 - 一键部署脚本（自包含）
# 用法：把本文件复制到服务器后执行 sudo bash deploy-latest.sh
...
REQUESTED_CLIENT_IMAGE_TAG="latest"
REQUESTED_SERVER_IMAGE_TAG="latest"
...
echo "    重新部署:  bash ${DEPLOY_DIR}/deploy-latest.sh"
echo ""
echo "  配置文件:  ${ENV_FILE}"
...
if [ -r "$SOURCE_SCRIPT" ] && cp "$SOURCE_SCRIPT" "${DEPLOY_DIR}/deploy-latest.sh"; then
  chmod +x "${DEPLOY_DIR}/deploy-latest.sh"
  log_info "部署脚本已保存: ${DEPLOY_DIR}/deploy-latest.sh"
else
