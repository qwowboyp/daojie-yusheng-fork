...
- 跨圖導航的首段使用玩家目前實例的完整傳送門清單，包含宗門核心的執行時出口；不能只讀地圖模板的固定界門。
- 路線預覽與實際尋路使用同一份出口清單。離開宗門後保留原目的地，沿公共地圖界門繼續導航。
- 執行時出口只參與目前實例的首段規劃，不寫回模板或全域路網；遷宗後下一次規劃直接讀取更新後的出口，其他宗門與同模板實例不會共用此出口。
