...
- 功法子分類沿用坊市的法術、內功、神通、秘術判斷與名稱；功法依模板 `realmLv`、其他物品依 `level` 排序，預設由低到高，同級按名稱與固定 ID 排列。未標示等級者在兩種等級排序中皆置後，設定等級上下限時不列入。
- 「條件篩選」可設定含端點的等級上下限，並與名稱、類型、功法子分類、未學狀態、來源及地圖取交集。上下限需為正整數，反向區間顯示錯誤；更改篩選或排序回到第一頁，直接開啟指定物品時清除篩選。手機篩選區可折疊與滾動，保留清單操作空間。
...
取得途徑百科以同步的已掌握功法清單顯示「已學」，可勾選「只看未學功法」。功法使用模板品階與境界等級（非修煉層數或學習硬門檻）；來源地圖使用地圖設定等級及同一境界目錄的推薦境界。自創功法通用書籍未綁定具體模板，不推測其已學狀態與境界。
