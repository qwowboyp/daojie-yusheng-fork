...
築基 31–42 級逐級新增十二部全卷，內功、法術、秘術與神通各三部，三張地圖對應十二／十四／十六個修煉層級。功法的境界等級與其可修煉層數是不同概念；既有《凝氣成基法》仍為 31 級、49 層，《血煞喚靈決》仍為 42 級、9 層。

正式內容由 `scripts/generate-foundation-techniques.mjs` 生成，只管理本批功法與指定書籍掉落。全卷不設定 `learnTechniqueMaxLevel`，沿用既有學書、補全殘卷與領悟流程；不直接重寫玩家已學狀態。新書進入既有坊市零掛單目錄，實際掛售仍由玩家提供。

新增內功沿用共用屬性預算；秘術與護持神通使用既有同類狀態 ID，避免跨境累積同類效果。取得怪物、機率與平衡邊界見 [築基功法與掉落](../../design/balance/築基功法與掉落.md)。

## 後期七境固定功法（2026-09-12）

金丹至渡劫各境新增兩部內功、兩部法術、兩部神通與兩部秘術，共 56 部全卷。
正式內容位於 `content/techniques/後期七境/玩家功法.json` 與 `content/items/後期七境/功法書.json`；怪物掉落由同一後期生成器合併寫入，不由執行期補表。

- 內功六維總量使用共用預算的 90% 或 100%，不同功法仍可累積；其餘新增分類不提供永久六維。
- 主動技能依 `unlockLevel` 解鎖，功法書可依現有領悟規則提前參悟；越階依現有領悟、修煉與靈力輸出超限懲罰處理，不新增硬性境界門禁。
- 秘術與護持神通的同類效果共用 `buffId`、`maxStacks: 1`，最後施放覆蓋前者，不因跨境學到多本而增加層數。技能增益沿用現有缺省境界的效力口徑；本批沒有新增境界衰減或共享冷卻機制。
- 內功預算、法術效率、神通輪替總爆發／回復量、秘術實際取整與百分比單位由 `scripts/prove-late-game-techniques.mjs` 驗證。

名稱、招式、地圖、掉落怪物與基礎／現世機率見 [後期七境功法與掉落](../../design/balance/後期七境功法與掉落.md)。此表按配置產生，平均取得次數不是保底，也不代表所有流派的實戰完成時間。
