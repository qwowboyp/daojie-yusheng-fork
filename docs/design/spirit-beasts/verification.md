...
## 融合 v2 本機驗證（2026-09-15，尚未發布）

規劃與實作：兩隻同品三星消耗融合為高一品一星，100% 成功；仙品不開放，四／五星不作為融合材料。沿用物種配對與較佳區間資質，後代技藝、戰力與速度按一星計算；不修改既有靈獸或升星規則。

- 共用規則：1,860 組可用配對逐一核對 CSV、v2 配方 ID、一星產出與親代順序；全部仙品配對拒絕，拒絕錯星、重複身份、不同所有者、受保護與工作中的親代。
- `pnpm audit:protocol` 通過，包含 shared 建置與 server 編譯；補充 CSV 斷言後另做 server 型別編譯及契約 smoke。
- 專用本機 PostgreSQL：真實命令預覽→融合→面板與 DB 回讀一致；錯星與仙品不能扣除，競爭融合只成功一次，親代消耗後重複請求不多發。
- client 型別檢查、直接 Vite 建置與 React 面板瀏覽器 proof 通過；驗證三星指令、一星預覽、候選排除與配方反查，涵蓋桌面明暗色、手機直向與觸控橫向模擬（4 張截圖）。修正 proof 等待離屏 lazy 圖片解碼的卡點，僅等待可見圖片並保留載入斷言與有界等待。
- 繁體文案與內容一致性檢查通過；測試 DB 已停止，本輪瀏覽器程序已關閉。
- 本輪定向證據集中於 `.runtime/reports/spirit-beast-fusion/`。未執行遊戲全測或生產發布；以下為先前 v1 的歷史發布紀錄。
